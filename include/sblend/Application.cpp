...
namespace sx
{
    ...
    void Application::keyCallback(Window window, int key, int scancode, int action, int mods)
    {
    ...
    void Application::resizeCallback(Window window, int width, int height)
    {
        ...
        WINDOW_WIDTH = width;
        projection = glm::perspective(glm::radians(fov), (float)WINDOW_WIDTH / (float)WINDOW_HEIGHT, 0.1f, 100.f);
    }
    ...
    void Application::updateMenuState()
    {
        if (mainMenu.selectedMesh != MeshType::NaN)
        {
            ...
            case MeshType::CIRCLE:
                if (mainMenu.generate)
                {
                    ...
                    mainMenu.existentMeshes.emplace_back("Circle " + std::to_string(mainMenu.meshTypeCounter[(int)mainMenu.selectedMesh]));
            ...
            case MeshType::SPHERE:
                if (mainMenu.generate)
                {
                    MeshManager::generateSphereMesh(vertices, indices, mainMenu.slices, mainMenu.stacks);
                    meshes->meshes.emplace_back(vertices, indices);
                    meshes->meshes.back().type = mainMenu.selectedMesh;
                    meshes->meshes.back().slices = mainMenu.slices;
                    meshes->meshes.back().stacks = mainMenu.stacks;
                    mainMenu.existentMeshes.emplace_back("Sphere " + std::to_string(mainMenu.meshTypeCounter[(int)mainMenu.selectedMesh]));
                    mainMenu.selectedMeshIndicator.emplace_back(0);
                    ++mainMenu.meshTypeCounter[(int)mainMenu.selectedMesh];
                    mainMenu.generate = false;
            ...
            case MeshType::TORUS:
                if (mainMenu.generate)
                {
                    MeshManager::generateTorusMesh(vertices, indices, mainMenu.slices, mainMenu.stacks);
                    meshes->meshes.emplace_back(vertices, indices);
                    meshes->meshes.back().type = mainMenu.selectedMesh;
                    meshes->meshes.back().slices = mainMenu.slices;
                    meshes->meshes.back().stacks = mainMenu.stacks;
                    mainMenu.existentMeshes.emplace_back("Torus " + std::to_string(mainMenu.meshTypeCounter[(int)mainMenu.selectedMesh]));
                    mainMenu.selectedMeshIndicator.emplace_back(0);
                    ++mainMenu.meshTypeCounter[(int)mainMenu.selectedMesh];
                    mainMenu.generate = false;
        ...
        {
            mainMenu.objectMenu.position = glm::value_ptr(meshes->meshes[mainMenu.selectedMeshIndex].position);
            mainMenu.objectMenu.velocity = glm::value_ptr(meshes->meshes[mainMenu.selectedMeshIndex].velocity);
            mainMenu.objectMenu.rotation = glm::value_ptr(meshes->meshes[mainMenu.selectedMeshIndex].angles);
            mainMenu.objectMenu.scale = glm::value_ptr(meshes->meshes[mainMenu.selectedMeshIndex].scale);
            mainMenu.objectMenu.color = glm::value_ptr(meshes->meshes[mainMenu.selectedMeshIndex].color);
            mainMenu.objectMenu.metallic = &meshes->meshes[mainMenu.selectedMeshIndex].metallic;
            mainMenu.objectMenu.roughness = &meshes->meshes[mainMenu.selectedMeshIndex].roughness;
            mainMenu.objectMenu.ao = &meshes->meshes[mainMenu.selectedMeshIndex].ao;
            renderObjectMenu = true;

            if(mainMenu.setTexture)
            {
                meshes->meshes[mainMenu.selectedMeshIndex].applyTexture( ("../../textures/" + std::string(mainMenu.texturePath)).c_str());
            ...
            if (mainMenu.deleteObject)
            {
                ...
                meshes->initialMeshesPositionsAndVelocities.erase(meshes->initialMeshesPositionsAndVelocities.begin() + mainMenu.selectedMeshIndex);
                mainMenu.existentMeshes.erase(mainMenu.existentMeshes.begin() + mainMenu.selectedMeshIndex);
                mainMenu.selectedMeshIndicator.erase(mainMenu.selectedMeshIndicator.begin() + mainMenu.selectedMeshIndex);
                renderObjectMenu = false;
            }
        }
        ...
        if (mainMenu.selectedLightIndex != -1)
        {
            ...
            if (mainMenu.deleteLight)
            {
                ...
                mainMenu.existentLights.erase(mainMenu.existentLights.begin() + mainMenu.selectedLightIndex);
                mainMenu.selectedLightIndicator.erase(mainMenu.selectedLightIndicator.begin() + mainMenu.selectedLightIndex);
                renderLightMenu = false;
            }
        }
            ...
            if (inputFile.is_open())
            {
                ...
                while (inputFile >> ty && ty != "LIGHTS")
                {
                    ...
                    case MeshType::PLANE:
                        MeshManager::generatePlaneMesh(vertices, indices);
                        meshes->meshes.emplace_back(vertices, indices);
                        mainMenu.existentMeshes.emplace_back("Plane " + std::to_string(mainMenu.meshTypeCounter[(int)mainMenu.selectedMesh]));
                        mainMenu.selectedMeshIndicator.emplace_back(0);
                        mainMenu.meshTypeCounter[(int)mainMenu.selectedMesh]++;
                        break;
                    ...
                    case MeshType::CUBE:
                        MeshManager::generateCubeMesh(vertices, indices);
                        meshes->meshes.emplace_back(vertices, indices);
                        mainMenu.existentMeshes.emplace_back("Cube " + std::to_string(mainMenu.meshTypeCounter[(int)mainMenu.selectedMesh]));
                        mainMenu.selectedMeshIndicator.emplace_back(0);
                        mainMenu.meshTypeCounter[(int)mainMenu.selectedMesh]++;
                        break;
                    ...
                    inputFile >> meshes->meshes.back().position.x >> meshes->meshes.back().position.y >> meshes->meshes.back().position.z;
                    inputFile >> meshes->meshes.back().velocity.x >> meshes->meshes.back().velocity.y >> meshes->meshes.back().velocity.z;
                    inputFile >> meshes->meshes.back().scale.x >> meshes->meshes.back().scale.y >> meshes->meshes.back().scale.z;
                    inputFile >> meshes->meshes.back().angles.x >> meshes->meshes.back().angles.y >> meshes->meshes.back().angles.z;
                    inputFile >> meshes->meshes.back().color.x >> meshes->meshes.back().color.y >> meshes->meshes.back().color.z;
                ...
                while (inputFile >> ty && ty != "Scene")
                {
                    ...
                    if (lightty == LightType::POINT)
                        ...
                        shaders->pbrLightningShader.setInt("lightCount", ++pbrLightCount);
                    inputFile >> lights.lights.back().position.x >> lights.lights.back().position.y >> lights.lights.back().position.z;
                    inputFile >> lights.lights.back().diffuse.x >> lights.lights.back().diffuse.y >> lights.lights.back().diffuse.z;
                    inputFile >> lights.lights.back().specular.x >> lights.lights.back().specular.y >> lights.lights.back().specular.z;
                    inputFile >> lights.lights.back().ambient.x >> lights.lights.back().ambient.y >> lights.lights.back().ambient.z;
                    inputFile >> lights.lights.back().color.x >> lights.lights.back().color.y >> lights.lights.back().color.z;
                    inputFile >> lights.lights.back().linear;
                    inputFile >> lights.lights.back().constant;
                    inputFile >> lights.lights.back().quadratic;
                    if (lightty == LightType::POINT)
                        mainMenu.existentLights.emplace_back("Point Light " + std::to_string(mainMenu.lightTypeCounter[(int)lightty]));
                    else
                        mainMenu.existentLights.emplace_back("PBR Light " + std::to_string(mainMenu.lightTypeCounter[(int)lightty]));
    ...
    void Application::updatePhysics()
    {
        ...
        if (mainMenu.simulationState)
        {
            ...
            {
                for (Mesh &mesh : meshes->meshes)
                    meshes->initialMeshesPositionsAndVelocities.push_back(std::make_pair(mesh.position, mesh.velocity));
    ...
    void Application::renderWithNoShadows()
    {
        if (windowResized)
        {
            ...
            mainMenu.objectMenu.menuPos = ImVec2((float)WINDOW_WIDTH - (float)WINDOW_WIDTH / 4.f, 0);
            mainMenu.objectMenu.menuSize = ImVec2((float)WINDOW_WIDTH / 4.f, (float)WINDOW_HEIGHT / 2.f);
            mainMenu.lightMenu.menuPos = ImVec2((float)WINDOW_WIDTH - (float)WINDOW_WIDTH / 4.f, (float)WINDOW_HEIGHT / 2.f);
            mainMenu.lightMenu.menuSize = ImVec2((float)WINDOW_WIDTH / 4.f, (float)WINDOW_HEIGHT / 2.f);
        }
        ...
        else if (mainMenu.usePbrLightShader)
        {
            ...
            shaders->pbrLightningShader.setVec3("camera.position", camera.getPosition());
    ...
    void Application::renderWithShadows()
    {
        ...
        shadow->lightView = glm::lookAt(
            lights.lights.front().position,
            glm::vec3(0.f),
    ...
    Application::Application() : camera(window, glm::vec3(0.f, 2.f, 3.f), 2.f)
    {
        ...
        window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "sBlend", nullptr, nullptr);
        ...
        appIcon.pixels = stbi_load("../../src/appIcon.png", &appIcon.width, &appIcon.height, nullptr, 0);
