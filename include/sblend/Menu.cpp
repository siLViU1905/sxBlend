...
void MainMenu::render() {
  ...
  if (ImGui::Button("Model"))
    modelBtnPressed = !modelBtnPressed;

  if (modelBtnPressed) {
    ImGui::Begin("Model loader");
    ImGui::InputText("Model path:", modelPath, 64);
    if (ImGui::Button("Ok")) {
      loadModel = true;
      modelBtnPressed = false;
      existentModels.push_back("Model " + std::to_string(modelCounter++));
      selectedModelIndicator.emplace_back(0);
    }
    ImGui::End();
  }
