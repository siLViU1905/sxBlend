...
bool Shader::compileShader(const char *filepath, unsigned int shaderType) {
  std::ifstream *file =
      new std::ifstream(filepath, std::ios::ate | std::ios::binary);
...
bool Shader::autoCompileAndLink(const char *vertexShaderFilepath,
...
bool Shader::isLocationValid(const char *name) const {
...
Shader::Shader() { vertexShader = fragmentShader = program = -1; }
